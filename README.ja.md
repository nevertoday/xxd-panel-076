...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 076 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
同一性、数、比率、動勢を固定 → 不要な背景密度を削除 → クレヨン、木炭、色鉛筆の粗い線で再構成 → 隠れたグリッドと非対称バランスで整理 → 約45%以上の連続余白 → 元写真の温かい色をマカロン色面へ再調整 → タイプライター観察注記
```

## 完成作の特徴

- 輪郭は途切れ、反復、ずれ、震えを許し、短いハッチング、擦れ、重ね線が局所的な重さをつくる。
- 紙目、蝋の摩擦、粒状の途切れ、乾いた縁、露出紙、不均一な被覆を残す。
- ガッシュ／アクリルは少量だけ使い、全面厚塗りにしない。マカロン色は明るく透明で甘すぎない。
- 主体を一領域に集中し、約45%以上の余白を空、空気、距離、水面、地面として働かせる。
- タイプライター小字は動勢や余白縁に沿う私的な観察注記で、通常の見出しではない。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-076-prompt.en.md)
...
| **兵士級** | `xxd-panel-NNN`（現在 001–076） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
