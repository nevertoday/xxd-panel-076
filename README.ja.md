...
# 🦁 XXD Panel 076｜マカロンクレヨン

### 粗い濃色クレヨンと明るいマカロン色で、写真を人間味ある旅の記憶へ
...
検証可能なオリジナル公開作例と出典が未提供のため、`sample-01`–`sample-04` は掲載していません。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 076 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な写真の手描き化は背景情報を残しすぎるか、「癒やし」のために安価なキャンディ色を当てがちです。その結果、主体の正確さも空間の呼吸も失われます。

**Panel 076** は濃色クレヨン、木炭、色鉛筆の粗い線で主体を探し直します。線は途切れ、反復、ずれ、擦れを許しつつ、重要な比率、方向、姿勢は正確です。背景を積極的に削り、約45%以上の連続した東洋的余白を確保し、元写真の温かく鮮やかな色の手掛かりを甘すぎない明るいマカロン色面へ再調整します。

### 向いている用途

- 旅や日常写真に、日光、衣服、花、壁から生まれる人間的な温かさを加えたい場合
- 省略しながらも主体の同一性、数、比率、方向、構造、姿勢を正確に保ちたい場合
- 汚れた暗色や安価なキャンディ色ではなく、透明で明るいマカロン色を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 076 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 温かく鮮やかで少し不器用ながら、成熟した出版秩序を持つ人間的な手描きポスター |
| **識別しやすい特徴** | 粗い濃色クレヨン、少量の明るいマットなマカロン色面、約45%以上の連続余白 |
| **入力の尊重** | 写真部は同一性と空間事実を忠実に保ち、転訳部は背景を省略して重要構造を固定する |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-076`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-076-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-076-prompt.zh-CN.md)
...
**特徴：** 粗い濃色クレヨン · 木炭と色鉛筆 · 明るいマカロン色面 · 45%連続余白 · 天然紙 · タイプライター観察注記
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 076 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-076 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-076 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 076 を適用し、共通設定は一度だけ解決
/xxd-panel-076 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
76 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 076 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
