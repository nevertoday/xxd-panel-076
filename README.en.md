...
No verifiable original-X samples or provenance have been supplied, so `sample-01`–`sample-04` are not displayed. The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 076 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Many photo-to-drawing treatments either preserve too much background density or force a cheap candy palette in the name of comfort. They lose both accurate identity and genuine spatial breathing room.

**Panel 076** finds the subject again through rough dark crayon, charcoal, and coloured-pencil lines that may break, repeat, shift, or rub while key proportion, direction, and pose stay accurate. It actively removes background noise, keeps roughly 45% or more continuous Eastern whitespace, and recomposes the source's warmest, most vivid clues as clear bright macaron gouache or acrylic blocks that never become sugary.

### Best for

- giving travel and everyday photographs more humane warmth through colours remembered from sunlight, clothes, flowers, and walls;
- omitting boldly while preserving identity, count, proportion, direction, structure, and pose;
- gaining bright restorative colour without forest-dark, dirty Morandi, nostalgic yellowing, fluorescent, or cheap-candy styling;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 076 on this image and recommend the most suitable composition and size first.
...
**Signature:** rough dark crayon · charcoal and coloured pencil · bright macaron blocks · 45% continuous whitespace · natural paper · typewriter observation notes
...
|---|---|
| **What you get** | A warm, vivid, slightly awkward humanist drawing governed by mature editorial order |
| **Recognisable signature** | Rough dark crayon, a few bright matte macaron blocks, and at least about 45% continuous whitespace |
| **How it respects the source** | The reality view preserves identity and spatial facts; the transformed region removes background density while locking key structure |
...
```text
lock identity, count, proportion, and movement → remove irrelevant background density → find form again with rough crayon, charcoal, and coloured pencil → organise with an implicit grid and asymmetric balance → preserve about 45% or more continuous Eastern whitespace → recompose warm vivid source clues as macaron blocks → add typewriter observation notes
```

## Recognisable finished traits

- Contours may break, repeat, shift, and tremble; short hatching, rubbing, and layered lines create local weight.
- Paper tooth, waxy friction, granular breaks, dry edges, exposed paper, and uneven coverage stay visible.
- Matte gouache or acrylic appears sparingly rather than as full impasto; macaron colour is clear and bright, never sugary.
- The subject occupies one clear zone while at least about 45% continuous whitespace acts as sky, air, distance, water, or ground.
- Small typewriter notes follow movement, whitespace edges, or an implicit grid like private observations rather than headlines.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-076-prompt.en.md)
...
```text
/xxd-panel-076 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-076 photo.jpg --prefs last
/xxd-panel-076 photo.jpg --prefs edit
/xxd-panel-076 photo.jpg --prefs new
```
...
/xxd-panel-076 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-076 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-076 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-076 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-076 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-076 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-076 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-076 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-076 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 076 to a whole directory; shared settings resolve once
/xxd-panel-076 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-076 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-076.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-076" ~/.codex/skills/xxd-panel-076
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-076 --skill xxd-panel-076
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-076`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-076-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-076-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

Each of the 76 Panels preserves its own original brief and aesthetic logic. The table continuously lists every project from 001 through 076 with its defining style characteristics; the current project is bold.
