...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 076 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

很多照片转手绘不是背景信息太满，就是为了“治愈”套用廉价糖果色；结果既失去主体准确性，也没有真正的空间呼吸。

**Panel 076** 以粗粝的深色蜡笔、炭笔和彩铅线重新寻找主体，允许断裂、重复、偏移与擦蹭，却牢牢保住关键比例、方向和姿态。画面主动删去多余背景，留下约 45% 以上连续东方留白；少量哑光水粉／丙烯色块从原图最温暖鲜活的线索重调为清透明亮、不过甜的马卡龙记忆。

### 适合这些情况

- **需要旅行与生活照片更有人情味：** 色彩像阳光、衣物、花朵和墙面留下的愉快记忆。
- **想保留准确身份又敢于省略：** 粗粝线条可以断裂和抖动，关键比例、方向与姿态不能走样。
- **不想要廉价糖果色：** 马卡龙色由原图温暖鲜活的线索重新调制，并由深色蜡笔稳定画面。
...
> 用 XXD Panel 076 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 粗粝深色蜡笔 · 炭笔与彩铅 · 明亮马卡龙色块 · 45% 连续留白 · 天然纸面 · 打字机观察注记

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 076 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅温暖、鲜活、略带笨拙却有成熟出版秩序的人文手绘海报 |
| **一眼可辨的特点** | 粗粝深色蜡笔线、少量明亮马卡龙哑光色块与至少约 45% 连续留白 |
| **它如何尊重你的输入** | 上部忠实保持身份、数量、姿态和空间；转译区省略背景但锁定关键比例、方向与关系 |
...
```text
锁定主体身份、数量、比例与动势 → 删除无关背景密度 → 以蜡笔／炭笔／彩铅粗粝线重新找形 → 用隐性网格与不对称平衡组织 → 保留约 45% 以上连续东方留白 → 从原图温暖鲜活线索重调马卡龙色块 → 以打字机微排版留下观察注记
```

## 成品中最容易识别的风格特点

- 轮廓允许断裂、重复、偏移和轻微抖动，短促排线、擦蹭与叠线形成局部重量。
- 天然纸齿、蜡质摩擦、颗粒断线、干涩边缘、露纸和不均匀覆盖都真实可见。
- 哑光水粉／丙烯只少量介入，不完整厚涂；马卡龙色明亮清透但不甜腻。
- 主体集中在明确区域，至少约 45% 连续留白代表天空、空气、距离、水面或地面。
- 打字机小字沿动势、留白边缘或隐性网格排布，像私人观察记录而非普通标题。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-076-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 076 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-076 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-076 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 076；公共设置只解析一次
/xxd-panel-076 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-076`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-076-prompt.en.md) · [中文运行适配器](references/xxd-panel-076-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

76 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–076 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–076） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
