...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 076이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
정체성·수·비율·동세 고정 → 불필요한 배경 밀도 삭제 → 크레용·목탄·색연필의 거친 선으로 재구성 → 암시적 그리드와 비대칭 균형 → 약 45% 이상의 연속 여백 → 원본의 따뜻한 색을 마카롱 색면으로 재조율 → 타자기 관찰 메모
```

## 완성작의 특징

- 윤곽은 끊김, 반복, 어긋남, 떨림을 허용하고 짧은 해칭, 문지름, 겹선이 국소 무게를 만듭니다.
- 종이 결, 왁스 마찰, 입자성 끊김, 마른 가장자리, 드러난 종이, 불균일 덮임을 남깁니다.
- 과슈／아크릴은 소량만 사용하고 전체 임파스토로 만들지 않습니다. 마카롱 색은 맑고 밝되 지나치게 달지 않습니다.
- 피사체를 한 영역에 집중하고 약 45% 이상의 여백을 하늘, 공기, 거리, 물 또는 땅으로 작동시킵니다.
- 타자기 작은 글자는 동세와 여백 가장자리를 따르는 사적 관찰 메모이며 일반 제목이 아닙니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-076-prompt.en.md)
...
| **병사급** | `xxd-panel-NNN` (현재 001–076) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
