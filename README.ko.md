...
# 🦁 XXD Panel 076｜마카롱 크레용

### 거친 짙은 크레용과 밝은 마카롱 색으로 사진을 인간적인 여행 기억으로 바꿉니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 076 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 076 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 076 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 076 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 076 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 076 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 076 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 076 additional top-bottom sample 4"></td>
  </tr>
</table>
...
검증 가능한 원본 공개 예시와 출처가 없어 `sample-01`–`sample-04`는 현재 표시하지 않습니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 076이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

흔한 사진 손그림은 배경 정보를 너무 많이 남기거나 ‘치유’를 위해 값싼 사탕색을 씌웁니다. 그 결과 피사체의 정확성과 공간의 호흡을 함께 잃습니다.

**Panel 076**는 짙은 크레용, 목탄, 색연필의 거친 선으로 피사체를 다시 찾습니다. 선은 끊기고 반복되고 어긋나고 문질러질 수 있지만 핵심 비율, 방향, 자세는 정확합니다. 배경을 적극적으로 덜어 약 45% 이상의 연속 동양적 여백을 남기고, 원본의 가장 따뜻하고 생생한 색 단서를 지나치게 달지 않은 밝은 마카롱 색면으로 재조율합니다.

### 이런 경우에 적합합니다

- 여행과 일상 사진에 햇빛, 옷, 꽃, 벽에서 나온 인간적인 온기를 더하고 싶을 때
- 과감히 생략하면서도 정체성, 수, 비율, 방향, 구조, 자세를 정확히 유지하고 싶을 때
- 탁한 암색이나 값싼 사탕색이 아닌 맑고 밝은 마카롱 색을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 076로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 따뜻하고 생생하며 약간 서툴지만 성숙한 출판 질서를 지닌 인간적 손그림 포스터 |
| **대표 특징** | 거친 짙은 크레용, 소량의 밝은 무광 마카롱 색면, 약 45% 이상의 연속 여백 |
| **입력 존중 방식** | 사진 영역은 정체성과 공간 사실을 충실히 보존하고, 변환 영역은 배경을 덜어 핵심 구조를 고정함 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-076`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-076-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-076-prompt.zh-CN.md)
...
**특징:** 거친 짙은 크레용 · 목탄과 색연필 · 밝은 마카롱 색면 · 45% 연속 여백 · 천연 종이 · 타자기 관찰 메모
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 076의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-076 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-076 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-076 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-076 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-076 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 076 적용, 공통 설정은 한 번만 확정
/xxd-panel-076 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
76개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 076까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
